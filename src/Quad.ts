...
export default class Quad extends Transform {
	...
	public draw(gl: WebGL2RenderingContext, delta_time: number, camera: Camera) {
		...
		this.transform.matrix = Matrix4x4.multiply(this.transform.matrix, Matrix4x4.from_rotation(Quaternion.from_euler(new Vector3(0, 1 * delta_time, 0))));
		...
		this.material.bind_uniform_data(
			{
				"u_matrix": { data: (Matrix4x4.multiply(camera.matrix, this.transform.matrix).to_array()) },
				"u_texture": { data: [0] }
			}
		);
